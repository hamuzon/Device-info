(function () {
  ...
  (function() {
    ...
    if(footerCopyright){
      if(config.link){ footerCopyright.innerHTML = `${yearText} <a href="${config.link}" target="_blank" rel="noopener noreferrer">${config.user}</a> device-info`; }
      else{ footerCopyright.textContent = `${yearText} device-info`; }
    }
  })();
})();
