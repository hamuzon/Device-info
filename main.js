...
function getOsBrowserByUA() {
  ...
  if (/Android/.test(ua)) {
    ...
    os=device==="iPhone"?"iOS":device==="iPad"?"iPadOS":"iOS";
  }
  ...
  else if (/Linux/.test(navigator.platform) || /Linux/.test(ua)) {
    os="Linux";
  ...
  else if (/SamsungBrowser/.test(ua)) browser="Samsung Browser", bver=(ua.match(/SamsungBrowser\/([\d\.]+)/)||[])[1]||bver;
  else if (/UC/.test(ua)) browser="UC Browser", bver=(ua.match(/UC?Browser\/([\d\.]+)/)||[])[1]||bver;
...
function updateCurrentTime() {
  ...
  for(const row of rows){
    if(row.firstElementChild?.textContent===dict[currentLang].now.replace(/<[^>]+>/g,'')){
